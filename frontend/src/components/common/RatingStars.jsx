export default function RatingStars({
  ...
  return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 6, flexWrap: "wrap" }} aria-label={label || (hasRating ? `Đánh giá ${displayValue} trên ${max}` : emptyLabel)}>
      ...
      {showValue && (
        <span style={{ fontSize: 12, color: textColor, fontWeight: 700 }}>
          {hasRating ? `${displayValue}${valueSuffix}` : emptyLabel}{count != null ? ` · ${count} đánh giá` : ""}
        </span>
      )}
    </div>
  );
