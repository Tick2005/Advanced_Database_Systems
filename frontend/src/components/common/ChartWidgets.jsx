...
// ─── Room rating distribution ───────────────────────────────────────────────
const RATING_BUCKET_COLORS = ["#16a34a", "#84cc16", "#eab308", "#f97316", "#ef4444"];

export function RatingDistributionChart({ rooms = [] }) {
  const buckets = [
    { name: "5★", min: 4.5, max: 5, value: 0 },
    { name: "4★", min: 3.5, max: 4.49, value: 0 },
    { name: "3★", min: 2.5, max: 3.49, value: 0 },
    { name: "2★", min: 1.5, max: 2.49, value: 0 },
    { name: "1★", min: 0, max: 1.49, value: 0 }
  ];

  rooms.forEach((room) => {
    const rating = Number(room.averageRating || 0);
    const bucket = buckets.find((item) => rating >= item.min && rating <= item.max) || buckets[buckets.length - 1];
    bucket.value += 1;
  });

  return (
    <div className="card-elevated" style={{ padding: 16 }}>
      <h3 style={{ marginTop: 0, marginBottom: 16 }}>Phân bố đánh giá phòng</h3>
      <ResponsiveContainer width="100%" height={300}>
        <PieChart>
          <Pie
            data={buckets}
          ...
          >
            {buckets.map((entry, index) => (
              <Cell key={`rating-cell-${entry.name}`} fill={RATING_BUCKET_COLORS[index % RATING_BUCKET_COLORS.length]} />
            ))}
          </Pie>
          <Tooltip contentStyle={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 10 }} />
        </PieChart>
