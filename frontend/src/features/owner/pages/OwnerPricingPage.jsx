  ...
  return (
    <section style={{ display: "grid", gap: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
        <div>
          <h1 style={{ margin: 0 }}>💵 Quản lý Pricing</h1>
          {currentBranch && <p style={{ margin: "4px 0 0", color: "#64748b", fontSize: 13 }}>Chi nhánh: <strong>{currentBranch.name}</strong> — {currentBranch.city}</p>}
        </div>
        <button className="btn btn-gold" onClick={openCreate} disabled={!can(ACTIONS.PRICING_CREATE) || !selectedBranchId}>
          ➕ Tạo pricing mới
        </button>
      </div>
      ...
      {roomTypes.length > 0 && (
        <div style={{ display: "grid", gap: 14 }}>
          {roomTypes.map((rt) => {
            ...
            return (
              <article key={rt.id} className="card" style={{ padding: 16, display: "grid", gap: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                  <div>
                  ...
                  <span style={{ fontSize: 12, color: "#94a3b8" }}>{rtPricings.length} chương trình</span>
                </div>
                {rtPricings.length > 0 ? (
                    ...
                    {rtPricings.map((p) => (
                      <div key={p.id} style={{
                        ...
                        <div>
                          ...
                          <span style={{ marginLeft: 8, fontSize: 12, color: "#64748b" }}>{p.startsOn} → {p.endsOn}</span>
                        </div>
                        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                          {p.discountPercent > 0 && (
                            <span style={{ background: "#dcfce7", color: "#16a34a", padding: "2px 10px", borderRadius: 99, fontSize: 12, fontWeight: 700 }}>
                              -{p.discountPercent}%
                            </span>
                          )}
                          <button className="btn" style={{ border: "1px solid #e2e8f0", background: "white", padding: "5px 10px", fontSize: 12 }} onClick={() => openEdit(p)}>✏️</button>
                        </div>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div style={{ fontSize: 13, color: "#94a3b8", fontStyle: "italic" }}>Chưa có chương trình pricing cho loại phòng này.</div>
                )}
              </article>
            );
          })}
      ...
      {roomTypes.length === 0 && rows.length > 0 && (
        <div style={{ display: "grid", gap: 8 }}>
          {rows.map((p) => (
            <article key={p.id} className="card" style={{ padding: 14, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              <div>
