  ...
  return (
    <section style={{ display: "grid", gap: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
        <div>
          <h1 style={{ margin: 0 }}>🏢 Quản lý Chi Nhánh</h1>
          <p style={{ margin: "4px 0 0", color: "#64748b", fontSize: 13 }}>{rows.length} chi nhánh đang hoạt động</p>
        </div>
        <button className="btn btn-primary" onClick={openCreate} disabled={!can(ACTIONS.BRANCH_CREATE)}>
          ➕ Tạo chi nhánh mới
        </button>
      </div>
      ...
      <div style={{ display: "grid", gap: 14, gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))" }}>
        {filtered.map((branch, idx) => (
          <article key={branch.id} className="card" style={{ overflow: "hidden", display: "grid" }}>
            <div style={{ position: "relative", height: 160 }}>
              ...
              <span style={{
                position: "absolute", bottom: 10, left: 14, fontWeight: 800, fontSize: 16,
            ...
            <div style={{ padding: 14, display: "grid", gap: 6 }}>
              <div style={{ fontSize: 13, color: "#64748b" }}>📍 {branch.address}, {branch.city}</div>
              <div style={{ fontSize: 13, color: "#64748b" }}>📞 {branch.phone || "—"} · ✉️ {branch.email || "—"}</div>
              <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                <button
                  className="btn"
                  style={{ flex: 1, border: "1px solid #cbd5e1", background: "white", padding: "7px 0", fontSize: 13 }}
