...
export default function StaffCheckoutPage() {
  ...
  return (
    <section className="card" style={{ padding: 18, display: "grid", gap: 10 }}>
      <h1 style={{ margin: 0 }}>Check-out booking</h1>
      <div>Booking ID: <span className="mono">{id}</span></div>
      <p style={{ margin: 0, color: "#64748b" }}>Xac nhan khach tra phong va ket thuc luu tru.</p>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <button className="btn btn-gold" onClick={submit} disabled={loading}>{loading ? "Dang xu ly..." : "Xac nhan check-out"}</button>
