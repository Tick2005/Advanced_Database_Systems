...
export default function PreviewBooking() {
  ...
  useBookingFunnelStep(BOOKING_STEPS.PREVIEW, { roomId: location.state?.roomId || null });
  ...
  const submitBooking = async () => {
    ...
    if (Object.keys(nextErrors).length > 0) {
      trackBookingStep("validation_failed", { step: BOOKING_STEPS.PREVIEW, errors: Object.keys(nextErrors) });
      return;
    }
    ...
    try {
      ...
      trackBookingStep("step_submit", { step: BOOKING_STEPS.PREVIEW, roomId: payload.roomId, branchId: payload.branchId, totalPrice: payload.totalPrice });
      const booking = await createBookingMutation.mutateAsync(payload);
      trackBookingStep("step_success", { step: BOOKING_STEPS.PREVIEW, bookingId: booking?.id || null });
      navigate(PATHS.CUSTOMER_BOOKING_REVIEW, { state: { booking, payload, room: selectedRoom } });
    } catch (error) {
      trackBookingStep("step_failed", { step: BOOKING_STEPS.PREVIEW, reason: normalizeBookingError(error) });
    }
  };
