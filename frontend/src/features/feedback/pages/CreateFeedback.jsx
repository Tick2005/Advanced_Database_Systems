...
export default function CreateFeedback() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setError(err.message || "Khong the tai du lieu feedback");
      } finally {
        setLoading(false);
      }
    };
  ...
  const validate = () => {
    const nextErrors = {};
    if (!form.bookingId) nextErrors.bookingId = "Vui long chon booking";
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setError(err.message || "Khong the gui feedback");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <section className="container" style={{ padding: "28px 24px" }}>
      ...
      <form className="card" style={{ padding: 18, display: "grid", gap: 12 }} onSubmit={submit}>
          ...
          <select value={form.bookingId} onChange={(event) => onBookingChange(event.target.value)}>
            ...
            {Array.from(bookingMap.values()).map((booking) => (
              <option key={booking.id} value={booking.id}>
                #{booking.id} - Room {booking.roomId} ({booking.checkInDate} - {booking.checkOutDate})
              </option>
            ))}
          </select>
        ...
        <div className="field">
          <label>Danh gia (1-5)</label>
          <input type="number" min={1} max={5} value={form.rating} onChange={(event) => onChange("rating", Number(event.target.value || 1))} />
