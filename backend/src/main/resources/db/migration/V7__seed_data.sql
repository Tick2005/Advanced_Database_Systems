...
VALUES
	...
	('22222222-2222-2222-2222-222222222222', 'Nguyen Van A - Manager DN', '0900000002', 'vi'),
	('22222222-2222-2222-2222-222222222223', 'Tran Thi B - Manager HCM', '0900000007', 'vi'),
	('33333333-3333-3333-3333-333333333333', 'Hoang Minh C - Staff DN', '0900000003', 'vi'),
	('33333333-3333-3333-3333-333333333334', 'Pham Tuan D - Staff HCM', '0900000008', 'vi'),
	('33333333-3333-3333-3333-333333333335', 'Vu Linh E - Staff HCM', '0900000009', 'vi'),
	('44444444-4444-4444-4444-444444444444', 'Demo Customer', '0900000004', 'vi')
ON CONFLICT (user_id) DO NOTHING;

-- Add demo avatars to user profiles for front-end display
UPDATE user_profiles SET avatar_url = CASE user_id
	WHEN '11111111-1111-1111-1111-111111111111' THEN 'https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg'
	WHEN '22222222-2222-2222-2222-222222222222' THEN 'https://images.pexels.com/photos/415829/pexels-photo-415829.jpeg'
	WHEN '33333333-3333-3333-3333-333333333333' THEN 'https://images.pexels.com/photos/2379005/pexels-photo-2379005.jpeg'
	WHEN '44444444-4444-4444-4444-444444444444' THEN 'https://images.pexels.com/photos/614810/pexels-photo-614810.jpeg'
	ELSE avatar_url
END
WHERE user_id IN ('11111111-1111-1111-1111-111111111111','22222222-2222-2222-2222-222222222222','33333333-3333-3333-3333-333333333333','44444444-4444-4444-4444-444444444444');
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_name = 'rooms' AND column_name = 'average_rating' AND data_type = 'numeric'
  ) THEN
    DROP VIEW IF EXISTS v_room_rating_snapshot_cte;
    DROP VIEW IF EXISTS v_top_rooms_by_rating;
    ALTER TABLE rooms ALTER COLUMN average_rating TYPE double precision USING average_rating::double precision;
  END IF;

  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'room_types' AND column_name = 'average_rating' AND data_type = 'numeric'
  ) THEN
    DROP VIEW IF EXISTS v_room_rating_snapshot_cte;
    DROP VIEW IF EXISTS v_top_rooms_by_rating;
    ALTER TABLE room_types ALTER COLUMN average_rating TYPE double precision USING average_rating::double precision;
  END IF;
END $$;
...
-- Thêm sau dòng cuối INSERT INTO services hiện tại (sau ON CONFLICT DO NOTHING)
INSERT INTO services (id, branch_id, code, name, description, price, service_mode, is_active)
VALUES
  ('88888888-8888-8888-8888-888888888891', '55555555-5555-5555-5555-555555555551', 'GYM-ACCESS', 'Gym & Fitness', 'Phòng gym hiện đại 24/7 với HLV cá nhân', 200000, 'BOTH', TRUE),
  ('88888888-8888-8888-8888-888888888892', '55555555-5555-5555-5555-555555555551', 'CITY-TOUR', 'City Tour', 'Tour thành phố nửa ngày có hướng dẫn viên', 650000, 'PREBOOK', TRUE),
  ('88888888-8888-8888-8888-888888888893', '55555555-5555-5555-5555-555555555552', 'BEACH-KAYAK', 'Kayak & Water Sports', 'Kayak, lướt ván, dù lượn miễn phí dành cho khách', 0, 'ON_SITE', TRUE),
  ('88888888-8888-8888-8888-888888888894', '55555555-5555-5555-5555-555555555552', 'SEAFOOD-BBQ', 'Seafood BBQ Dinner', 'Tiệc BBQ hải sản tươi sống bên bãi biển', 950000, 'ON_SITE', TRUE),
  ('88888888-8888-8888-8888-888888888895', '55555555-5555-5555-5555-555555555551', 'ROOM-FLOWERS', 'Room Decoration', 'Trang trí phòng hoa tươi, nến, champagne', 500000, 'PREBOOK', TRUE)
ON CONFLICT (id) DO NOTHING;
...
UPDATE services SET thumbnail_url = CASE code
  WHEN 'BF-SET'         THEN 'https://images.pexels.com/photos/262978/pexels-photo-262978.jpeg'
  WHEN 'AIRPORT-PICKUP' THEN 'https://images.pexels.com/photos/1252983/pexels-photo-1252983.jpeg'
  WHEN 'LAUNDRY-FAST'   THEN 'https://images.pexels.com/photos/3861487/pexels-photo-3861487.jpeg'
  WHEN 'SPA-DELUXE'     THEN 'https://images.pexels.com/photos/3757942/pexels-photo-3757942.jpeg'
  WHEN 'DINNER-5C'      THEN 'https://images.pexels.com/photos/262978/pexels-photo-262978.jpeg'
  WHEN 'PRIVATE-TAXI'   THEN 'https://images.pexels.com/photos/4481326/pexels-photo-4481326.jpeg'
  WHEN 'POOL-SUNSET'    THEN 'https://images.pexels.com/photos/3757657/pexels-photo-3757657.jpeg'
  WHEN 'KIDS-CARE'      THEN 'https://images.pexels.com/photos/4269700/pexels-photo-4269700.jpeg'
  ELSE thumbnail_url
END
WHERE code IN ('BF-SET','AIRPORT-PICKUP','LAUNDRY-FAST','SPA-DELUXE','DINNER-5C','PRIVATE-TAXI','POOL-SUNSET','KIDS-CARE');

-- Additional services for richer demo
INSERT INTO services (id, branch_id, code, name, description, price, service_mode, is_active)
VALUES
	('88888888-8888-8888-8888-888888888884', '55555555-5555-5555-5555-555555555551', 'SPA-DELUXE', 'Spa Deluxe', 'Liệu trình spa 90 phút với tinh dầu cao cấp', 850000, 'ON_SITE', TRUE),
	('88888888-8888-8888-8888-888888888885', '55555555-5555-5555-5555-555555555551', 'DINNER-5C', 'Fine Dining 5 courses', 'Bữa tối cao cấp 5 món', 1200000, 'ON_SITE', TRUE),
	('88888888-8888-8888-8888-888888888886', '55555555-5555-5555-5555-555555555552', 'PRIVATE-TAXI', 'Private Taxi', 'Xe riêng chất lượng cao đón/trả theo yêu cầu', 400000, 'PREBOOK', TRUE),
	('88888888-8888-8888-8888-888888888887', '55555555-5555-5555-5555-555555555551', 'POOL-SUNSET', 'Sunset Pool Access', 'Vé hồ bơi tầng mái, view hoàng hôn', 0, 'ON_SITE', TRUE),
	('88888888-8888-8888-8888-888888888888', '55555555-5555-5555-5555-555555555552', 'KIDS-CARE', 'Kids Care', 'Dịch vụ trông trẻ chuyên nghiệp theo giờ', 300000, 'ON_SITE', TRUE)
ON CONFLICT (id) DO NOTHING;
...
-- THAY TOÀN BỘ BLOCK UPDATE thumbnail_url bằng đoạn sau:
UPDATE services
SET thumbnail_url = CASE code
  ...
  WHEN 'SPA-DELUXE'     THEN 'https://images.pexels.com/photos/3825586/pexels-photo-3825586.jpeg'
  WHEN 'DINNER-5C'      THEN 'https://images.pexels.com/photos/302899/pexels-photo-302899.jpeg'
  WHEN 'PRIVATE-TAXI'   THEN 'https://images.pexels.com/photos/1149831/pexels-photo-1149831.jpeg'
  WHEN 'POOL-SUNSET'    THEN 'https://images.pexels.com/photos/261102/pexels-photo-261102.jpeg'
  WHEN 'KIDS-CARE'      THEN 'https://images.pexels.com/photos/3662666/pexels-photo-3662666.jpeg'
  ELSE thumbnail_url
END
WHERE code IN ('BF-SET','AIRPORT-PICKUP','LAUNDRY-FAST','SPA-DELUXE','DINNER-5C','PRIVATE-TAXI','POOL-SUNSET','KIDS-CARE');
...
-- Thêm bookings đa dạng hơn
INSERT INTO bookings (id,customer_id,room_id,branch_id,check_in_date,check_out_date,adults,children,total_price,status,source_channel,confirmed_at)
VALUES
  ('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa6','44444444-4444-4444-4444-444444444447','77777777-7777-7777-7777-777777777774','55555555-5555-5555-5555-555555555552',DATE '2026-05-15',DATE '2026-05-17',2,0,3100000,'CONFIRMED','WEB',NOW()),
  ('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa7','44444444-4444-4444-4444-444444444448','77777777-7777-7777-7777-777777777775','55555555-5555-5555-5555-555555555551',DATE '2026-05-03',DATE '2026-05-06',2,1,3840000,'CHECKED_OUT','WEB',NOW()-INTERVAL '10 day'),
  ('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa8','44444444-4444-4444-4444-444444444444','77777777-7777-7777-7777-777777777778','55555555-5555-5555-5555-555555555552',DATE '2026-05-20',DATE '2026-05-22',1,0,3240000,'CONFIRMED','WEB',NOW())
ON CONFLICT (id) DO NOTHING;

INSERT INTO booking_services (id,booking_id,service_id,quantity,actual_price)
VALUES
  ('bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbb2','aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa6','88888888-8888-8888-8888-888888888886',1,400000),
  ('bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbb3','aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa7','88888888-8888-8888-8888-888888888881',2,300000),
  ('bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbb4','aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa8','88888888-8888-8888-8888-888888888884',1,850000)
ON CONFLICT (id) DO NOTHING;
...
-- ============================================================
-- V7 ADDENDUM: Seed thêm dữ liệu phong phú cho kiểm thử
-- Commit: feat(seed): bổ sung feedbacks, pricing logs, thêm
--         bookings đa trạng thái, camera settings mặc định
-- ============================================================

-- Feedback mẫu
INSERT INTO feedbacks (id, booking_id, customer_id, room_id, rating, comment, created_at)
VALUES
  ('ff000001-0000-0000-0000-000000000001', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa3', '44444444-4444-4444-4444-444444444446', '77777777-7777-7777-7777-777777777773', 5, 'Phòng rất sạch, nhân viên nhiệt tình, sẽ quay lại!', NOW() - INTERVAL '4 day'),
  ('ff000002-0000-0000-0000-000000000002', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa7', '44444444-4444-4444-4444-444444444448', '77777777-7777-7777-7777-777777777775', 4, 'View đẹp, dịch vụ tốt. Chỉ hơi ồn ào buổi sáng.', NOW() - INTERVAL '8 day')
ON CONFLICT (id) DO NOTHING;

-- Pricing logs mẫu (Owner approval trail)
INSERT INTO pricing_logs (id, branch_id, pricing_request_id, action, actor_id, note, created_at)
VALUES
  ('ee000001-0000-0000-0000-000000000001', '55555555-5555-5555-5555-555555555551', 'dd000001-0000-0000-0000-000000000001', 'APPROVED', '11111111-1111-1111-1111-111111111111', 'Approved for Hung Kings Festival', NOW() - INTERVAL '6 day'),
  ('ee000002-0000-0000-0000-000000000002', '55555555-5555-5555-5555-555555555551', 'dd000003-0000-0000-0000-000000000003', 'REJECTED', '11111111-1111-1111-1111-111111111111', 'Conflicts with summer rates', NOW() - INTERVAL '1 day')
ON CONFLICT (id) DO NOTHING;

-- MongoDB customer settings defaults được xử lý ở init-hotel.js
-- (giá trị allowCamera=true, allowLocation=true là mặc định)
