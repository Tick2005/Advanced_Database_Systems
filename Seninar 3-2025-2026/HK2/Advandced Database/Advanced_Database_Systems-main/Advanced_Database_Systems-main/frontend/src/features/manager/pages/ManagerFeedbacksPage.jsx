  ...
  return (
    <section style={{ display: "grid", gap: 16 }}>
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 12, opacity: 0.7, marginBottom: 4 }}>Phản hồi khách hàng</div>
          <div style={{ fontWeight: 800, fontSize: 20 }}>Feedback & Đánh giá</div>
          <div style={{ fontSize: 13, opacity: 0.75, marginTop: 2 }}>Quản lý và trả lời phản hồi của khách</div>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 14, display: "grid", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <label style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer", fontSize: 13 }}>
            <input type="radio" name="rating" value="ALL" checked={ratingFilter === "ALL"} onChange={(e) => setRatingFilter(e.target.value)} style={{ cursor: "pointer" }} />
            Tất cả
          </label>
          {[5, 4, 3, 2, 1].map((r) => (
            <label key={r} style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer", fontSize: 13 }}>
              <input type="radio" name="rating" value={String(r)} checked={ratingFilter === String(r)} onChange={(e) => setRatingFilter(e.target.value)} style={{ cursor: "pointer" }} />
              {r === 5 ? "⭐⭐⭐⭐⭐" : r === 4 ? "⭐⭐⭐⭐" : r === 3 ? "⭐⭐⭐" : r === 2 ? "⭐⭐" : "⭐"}
            </label>
          ))}
        </div>
          ...
          {visibleFeedbacks.map((fb) => (
            <article key={fb.id} className="card" style={{ padding: 16, display: "grid", gap: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8 }}>
                ...
                <div style={{ fontWeight: 800, fontSize: 15, color: STAR_COLORS[fb.rating] || "#94a3b8" }}>
                  <RatingStars value={fb.rating} size={15} showValue />
                </div>
              </div>
