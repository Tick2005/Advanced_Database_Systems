// commit: fix(vnpay-payment): sửa encoding tiếng Việt
...
export default function VnPayPayment() {
  ...
  const startPayment = async () => {
    ...
    if (existingLock === String(booking.id)) {
      setError("Yêu cầu thanh toán đang được xử lý, vui lòng đợi trong giây lát.");
      return;
    }
  ...
  if (!booking?.id) {
    return (
      <section className="container" style={{ padding: "28px 24px" }}>
        <p>Không có booking để thanh toán.</p>
  ...
  return (
    <section className="container" style={{ padding: "28px 24px" }}>
      ...
      <div className="card" style={{ padding: 18, display: "grid", gap: 10 }}>
        ...
        <button className="btn btn-gold" onClick={startPayment} disabled={loading} aria-label="Xác nhận và thanh toán qua VNPay">
          {loading ? "Đang chuyển hướng..." : "Xác nhận và thanh toán VNPay"}
        </button>
      </div>
    </section>
  );
