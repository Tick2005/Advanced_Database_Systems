  ...
  return (
    <section style={{ display: "grid", gap: 16 }}>
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 12, opacity: 0.7, marginBottom: 4 }}>Định giá & yêu cầu</div>
          <div style={{ fontWeight: 800, fontSize: 20 }}>Pricing Requests</div>
          {branch && <div style={{ fontSize: 13, opacity: 0.75, marginTop: 2 }}>{branch.name}</div>}
        </div>
      </div>
      {false && (
        <div>
      ...
      <div style={{ display: "grid", gap: 12 }}>
        ...
        {rows.map((row) => (
          <article key={row.id} className="card" style={{ padding: "14px 16px", display: "grid", gap: 10 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                {row.discountPercent > 0 && (
                  <span style={{ background: "#fef9c3", color: "#854d0e", padding: "4px 12px", borderRadius: 6, fontSize: 12, fontWeight: 700 }}>
                    -{row.discountPercent}%
                  </span>
                )}
